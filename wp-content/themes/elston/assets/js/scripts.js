jQuery(document).ready(function ($) {
  ...
  if (screen.width >= 1280) {
    $(".clickevent .toggle-link, .clickevent .grid-link").on(
      "click",
      function () {
  ...
  $(document).on("click", function (e) {
    if (
      $(e.target).is(
        ".elstn-sidebar, .elstn-sidebar *, .elstn-toggle-btn, .elstn-toggle-btn *"
      ) === false
    ) {
  ...
  function parallax() {
    ...
    $(".elstn-parallax-text .banner-caption").css(
      "opacity",
      1 - scrolled / 950
    );
  ...
  $(window).load(function () {
    $(".elstn-default-slider").each(function () {
      ...
      $carousel.owlCarousel({
        ...
        nav:
          $carousel.data("nav") !== undefined ? $carousel.data("nav") : false,
        navText: [
  ...
  $(".elstn-blog-sticky .col-md-4").theiaStickySidebar({
  ...
  $(
    ".elstn-portfolio-picture, .elstn-products-wrap, .gallery-item"
  ).magnificPopup({
    delegate: "a",
    type: "image",
    ...
    if (jQuery(".elstn-floting-wrap").length > 0) {
      if ($window.width() > 1023) {
        if ($window.scrollTop() + $topHeight > offset.top) {
          if (
            $window.scrollTop() + $topHeight + $flotingbar.height() + 50 <
            $offsetHeight.top + $scrolling
          ) {
  ...
  $(window).scroll(function () {
    var $window = jQuery(window),
      $flotingbar = jQuery(".stickysidebar"),
      offset = jQuery(
        ".elstn-portfolio-detail .vc_row, .elstn-blog-detail .wrapper"
      ).offset(),
      $scrolling = jQuery(
        ".elstn-portfolio-detail .vc_col-sm-8, .elstn-blog-detail .col-md-8"
      ).height(),
      $offsetHeight = jQuery(
        ".elstn-portfolio-detail .vc_col-sm-8, .elstn-blog-detail .col-md-8"
      ).offset(),
  ...
  if (tr) {
    $(".elstn-portfolio-detail .sticky")
      .parents(".wpb_column.vc_column_container")
      .addClass("stickysidebar");
    $(".elstn-portfolio-detail .floting")
      .parents(".wpb_column.vc_column_container")
      .removeClass("stickysidebar");
  }

  if (tr1) {
    $(".elstn-portfolio-detail .floting")
      .parents(".wpb_column.vc_column_container")
      .addClass("elstn-floting-wrap");
    $(".elstn-portfolio-detail .sticky")
      .parents(".wpb_column.vc_column_container")
      .removeClass("elstn-floting-wrap");
  }

  $(".elstn-portfolio-detail .sticky")
    .parents(".wpb_column.vc_column_container")
    .addClass("stickysidebar");
  $(".elstn-portfolio-detail .elstn-floting-wrap")
    .parents(".wpb_column.vc_column_container")
    .removeClass("stickysidebar");
  ...
  $(".elstn-projects-slider")
    .parents(".elstn-portfolio-detail")
    .find(".elstn-more-project")
    .addClass("spacer2");

  //Custom Code
  $("div.vc_tta-tabs-container")
    .children("ul.vc_tta-tabs-list")
    .removeClass("vc_tta-tabs-list")
  ...
  $(".elstn-services")
    .find(".vc_row > .wpb_column")
    .addClass("margin-no padding-no");
  $(".elstn-services-group").addClass("wrapper");

  $(".elstn-btn.submit").on("click", function () {
    function showpanel() {
      var auth_field = $(
          'form.comment-form .elstn-form-inputs label[for="author"]'
        ),
        email_field = $(
          'form.comment-form .elstn-form-inputs label[for="email"]'
        ),
        has_auth_attr = auth_field.attr("style");
      has_email_attr = email_field.attr("style");

      if (has_auth_attr == "display: none;") {
        $('form.comment-form .elstn-form-inputs input[name="author"]').css(
          "border-color",
          "#e8e8e8"
        );
      } else {
      ...
      if (has_email_attr == "display: none;") {
        $('form.comment-form .elstn-form-inputs input[name="email"]').css(
          "border-color",
          "#e8e8e8"
        );
      } else {
  ...
  $('form.comment-form .elstn-form-inputs input[name="author"]').keyup(
    function () {
      $(this).css("border-color", "#e8e8e8");
    }
  );

  $('form.comment-form .elstn-form-inputs input[name="email"]').keyup(
    function () {
      $(this).css("border-color", "#e8e8e8");
    }
  );
